...
export const handler = ({ inputs, mechanic }) => {
  ...
  useEffect(() => {
    if (logo) loadImageFromFileObject(logo, setLogoHref);
    if ((logo && logoHref) || (!logo && !logoHref)) mechanic.done();
  }, [logoHref, logo]);
      ...
      >
        ...
        <g fill={text_color} fontFamily="SyneVariable">
          ...
          <text
            ...
            letterSpacing="-.02em"
          >
            <tspan x={91} y={263} className="tabular-nums">
              {version}
            </tspan>
          </text>
        </g>
...
export const inputs = {
  title: { type: "text", default: "FreiØr" },
  weight: {
    type: "number",
    min: 400,
    max: 800,
...
export const ExtraUI = (props) => {
  ...
  return <button onClick={clickHandler}>Randomize columns!</button>;
};
